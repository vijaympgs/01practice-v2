...
try:
    from .views import (
        POSFunctionViewSet, 
        RolePOSFunctionMappingViewSet, 
        UserLocationMappingViewSet, 
        BulkUserLocationMappingView,
        UserAccessibleLocationsView,
        UserDefaultLocationView,
        SyncUserLocationMappingsView
    )
except ImportError:
    POSFunctionViewSet = None
    RolePOSFunctionMappingViewSet = None
    UserLocationMappingViewSet = None
    BulkUserLocationMappingView = None
    UserAccessibleLocationsView = None
    UserDefaultLocationView = None
    SyncUserLocationMappingsView = None
...
if RolePOSFunctionMappingViewSet:
    router.register(r'role-pos-function-mappings', RolePOSFunctionMappingViewSet, basename='role-pos-function-mappings')

# Register User Location Mapping viewsets if available
if UserLocationMappingViewSet:
    router.register(r'user-location-mappings', UserLocationMappingViewSet, basename='user-location-mapping')
...
urlpatterns = [
    ...
    # User Location Mapping bulk operations
    path('user-location-mappings/bulk/', BulkUserLocationMappingView.as_view(), name='bulk-user-location-mappings'),
    
    # Role-based location access endpoints
