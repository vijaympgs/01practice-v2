...
def test_day_open_api(headers):
    ...
    if response.status_code == 200:
        ...
        print("INFO: No active day open found")
        return None
    else:
        print(f"ERROR: Error checking active day open: {response.text}")
        return None
...
def test_session_api(headers):
    ...
    if response.status_code == 200:
        ...
        print(f"ERROR: Error checking sessions: {response.text}")
        return None
...
def test_terminal_api(headers):
    ...
    if response.status_code == 200:
        ...
        print(f"SUCCESS: Found {len(terminals)} terminals")
        ...
        print(f"ERROR: Error checking terminals: {response.text}")
        return []
...
def test_theme_api():
    ...
    if response.status_code == 200:
        data = response.json()
        print("SUCCESS: Active theme found:")
        print(f"   - Primary Color: {data.get('primary_color')}")
        print(f"   - Theme Name: {data.get('name')}")
        return data
    else:
        print(f"ERROR: Error checking theme: {response.text}")
        return None
...
def test_user_location_api(headers):
    ...
    if response.status_code == 200:
        ...
        print(f"ERROR: Error checking user profile: {response.text}")
        return None
...
def test_day_end_workflow_simulation(headers):
    ...
    if response.status_code == 200:
        ...
        print(f"SUCCESS: Sales API accessible - Found {len(sales)} sales records")
    else:
        print(f"WARNING: Sales API not available: {response.text}")
    ...
    if response.status_code == 200:
        ...
        print(f"SUCCESS: Products API accessible - Found {len(products)} products")
    else:
        print(f"WARNING: Products API not available: {response.text}")
...
def main():
    ...
    print(f"Day Open API: Working" if day_open else "Day Open API: No active day")
    print(f"Session API: Working" if active_session is not None else "Session API: No active sessions")
    ...
    if day_open:
        print("SUCCESS: Day Open component should show active day status")
    else:
        print("INFO: Day Open component should show 'No active day' state")
    
    if terminals:
        print("SUCCESS: Session Open component should have terminals available")
    else:
        print("ERROR: Session Open component may have terminal selection issues")
    
    if active_session:
        print("SUCCESS: Console should show active session status")
    else:
        print("INFO: Console should show 'No active session' state")
