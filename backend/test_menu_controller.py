#!/usr/bin/env python
"""
Test script for Django Admin Menu Controller
...
def test_menu_items_endpoint(token):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Menu Items API working!")
            ...
            print(f"❌ Menu Items API failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        print(f"❌ Menu Items API error: {e}")
        return False
...
def test_menu_visibility_endpoint(token):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Menu Visibility API working!")
            ...
            print(f"❌ Menu Visibility API failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        print(f"❌ Menu Visibility API error: {e}")
        return False
...
def test_user_menu_permissions_endpoint(token):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ User Menu Permissions API working!")
            ...
            print(f"❌ User Menu Permissions API failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        print(f"❌ User Menu Permissions API error: {e}")
        return False

def test_menu_statistics_endpoint(token):
    """Test menu statistics API endpoint (admin only)"""
    print("\n🔍 Testing Menu Statistics API Endpoint...")
    
    try:
        headers = {'Authorization': f'Bearer {token}'}
        response = requests.get(f'{API_BASE_URL}/users/menu-statistics/', headers=headers)
        
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Menu Statistics API working!")
            print(f"   - Total items: {data.get('overview', {}).get('total_menu_items', 0)}")
            print(f"   - Active items: {data.get('overview', {}).get('active_menu_items', 0)}")
            print(f"   - User permissions: {data.get('permissions', {}).get('user_permissions', 0)}")
            print(f"   - Group permissions: {data.get('permissions', {}).get('group_permissions', 0)}")
            return True
        else:
            print(f"❌ Menu Statistics API failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        print(f"❌ Menu Statistics API error: {e}")
        return False
...
def test_database_models():
    ...
    try:
        ...
        print(f"✅ MenuItemType model working!")
        ...
        print(f"✅ UserPermission model working!")
        ...
        print(f"✅ GroupPermission model working!")
...
def test_django_admin_setup():
    ...
    try:
        ...
        print(f"❌ Django admin setup error: {e}")
        return False
...
def test_menu_visibility_update(token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Menu visibility update working!")
            print(f"   - Updated items: {result.get('updated_count', 0)}")
            print(f"   - Message: {result.get('message', '')}")
            return True
        else:
            print(f"❌ Menu visibility update failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        print(f"❌ Menu visibility update error: {e}")
        return False
...
def main():
    ...
    if token:
        ...
        # Test 7: Menu Statistics API
        results.append(test_menu_statistics_endpoint(token))
        
        # Test 8: Menu Visibility Update
