...
"""
Setup script for demo locations and users
...
def check_current_state():
    """Check current companies, locations, and users"""
    print("=== CURRENT STATE ===")
    ...
    for loc_type, _ in Location.LOCATION_TYPE_CHOICES:
        type_locations = locations.filter(location_type=loc_type)
        print(f"  {loc_type.upper()}: {type_locations.count()}")
    ...
    for role, _ in User.ROLE_CHOICES:
        ...
        print(f"  {role.upper()}: {role_users.count()}")
        for user in role_users:
            print(f"    - {user.username} (pos_location: {user.pos_location})")
    
    # Check user-location mappings
    mappings = UserLocationMapping.objects.all()
    print(f"\nUSER-LOCATION MAPPINGS: {mappings.count()}")
...
if __name__ == '__main__':
    ...
    response = input("\nDo you want to create demo data? (y/n): ").lower().strip()
    if response == 'y':
        create_demo_data()
    else:
